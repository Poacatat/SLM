...
void Generator::generate(int input_word_size, const string& filenamem inte output_word_zise){
    Model model(input_word_size);
    model.train(filename);
    string current_word = pick_start_word();

    cout << "begin " << endl<<current_word;
    while (chars_printed<output_word_zise){
        char next_char = calculate_next_char(current_word);
        cout<<next_char;
